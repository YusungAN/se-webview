...
const MyFound = () => {
    ...
    const getList = async () => {
        try {            
            const response = await axios.get(`http://3.37.242.189:8000/profile/report_list/?user_id=${localStorage['user_id']}`);
            ...
            <div style={Wrapper}>
                ...
                findList.map((data, idx) => (
                        <div key={idx} style={{color: 'black', width: '100%', display: 'flex', alignItems: 'center', height: '130px', borderBottom: '1px solid black', paddingLeft: '30px'}}>
                            {data.photo != null ? <img src={`http://3.37.242.189/${data.photo}`} width={50} height={50} style={{border: '1px solid black'}} /> : <div style={{width: '50px', height: '50px', backgroundColor: 'grey', textAlign: 'center'}}>no photo</div>}
                            <div>
